...
The screenshots below were generated by running `tools/java/smartfm/ui/gui/ScreenshotDriver.java`. The driver automates user actions and captures the application window directly. The full set of 28 screenshots (covering home screen, valid/invalid inputs, outputs, exit state, and compilation evidence) is stored in `images/`. The figures below illustrate the core implementation evidence.
