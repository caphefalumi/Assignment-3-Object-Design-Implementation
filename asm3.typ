...
Building SmartFM provided key insights into the practical application of object-oriented design patterns, GRASP principles, and layered architecture.

*Specify Observer semantics and GRASP Controller boundaries precisely.* The State pattern lifecycle tables from Assignment 2 were our most effective design artifact because each row defined strict state transition invariants, making `OrderState` and `ShipmentState` implementation straightforward. However, the Observer pattern specifications were less precise. The vague event description for `OrderApprovedListener` caused debate over whether `DispatchManager` (a GRASP Application Controller) should trigger automatic vehicle assignment upon event notification or wait for explicit Presentation Layer invocation. In future designs, event-driven decoupling must specify explicit trigger conditions, payload contracts, and controller invocation boundaries.

*Validate domain multiplicities and interaction diagrams with edge-case scenarios.* Tracing dynamic interaction scenarios through sequence diagrams is essential for catching structural flaws early. The 1-to-1 multiplicity constraint between `Invoice` and `Payment` broke when implementing partial settlement (e.g., \$200 cash deposit followed by \$300 card settlement). Walking a multi-payment scenario through the `Invoice` domain entity (the Information Expert for billing balance) during Assignment 2 would have highlighted the need for a 1-to-Many multiplicity and an `InvoicePartiallyPaidState` prior to coding.

*Align GoF Patterns with Indirection and Protected Variations.* We designed `IPricingStrategy` and `PricingTariff` to encapsulate pricing algorithms (Strategy pattern and Protected Variations). However, during implementation, `OrderProcessor` invoked `PricingTariff.calculateQuote()` directly rather than delegating through `ServiceOffering` (the Information Expert for branch service offerings). Leaving a GoF pattern structurally declared but bypassed in runtime control flow creates redundant Indirection without benefit. Future architectures should enforce end-to-end pattern invocation or document deferred delegation points.

*Establish Architectural Layering and Persistence Contracts early.* Deferring UI and persistence details in Assignment 2 was appropriate for high-level modeling, but it left critical architectural boundaries unmapped. Implementing the Infrastructure Layer (`DataStore` as a Pure Fabrication gateway) required defining transaction boundaries, schema versioning, and input validation in `smartfm.common.Validators` without explicit design guidelines. Defining a clear Layered Architecture contract (Presentation -> Application -> Domain -> Infrastructure) and basic data boundaries during initial design minimizes interpretation drift during implementation.

*Verify Architectural Invariants through Automated Regression Suites.* Automated JUnit 5 testing (76 test executions across 17 test classes) proved vital for preserving domain invariants across refactoring iterations. Whenever we updated `DataStore` JDBC routines or added `ReportProcessor` metrics, automated unit and integration tests instantly verified that domain state rules and GRASP Controller workflows remained unviolated. Furthermore, deploying on Java 26 highlighted the infrastructure requirement of explicit JVM flags (`--enable-native-access=ALL-UNNAMED`) for native SQLite JDBC library binding.
