...
*Development environment:* Development was conducted on Windows using PowerShell 7.6 and OpenJDK 26.0.2. The application requires Java 26 and the bundled library JARs in `lib/`. It runs locally without external database servers. Build options include Maven, GNU Make, or standard `javac`.
...
#figure(
  console(raw(read("transcripts/00_compilation_evidence.txt"), lang: "text")),
